...
namespace EMX.HierarchyPlugin.Editor.Windows
{
	...
	public class InputWindow : IWindow
	{
		...
		internal static IWindow Init(MousePos rect, string title, Window adapter, Action<string> conform, Action<string> close = null, string textInputSet = "",
			float destroy = -1) //  Debug.Log("ASD");
		{
			...
			if (conform == null)
			{
				...
				Root.p[0].GET_SKIN().label.fontSize = Root.p[0].WINDOW_FONT_10();
				float w1, w2;
				Root.p[0].GET_SKIN().label.CalcMinMaxWidth(new GUIContent(textInputSet + " "), out w1, out w2);
				var targetW = w1 + 10;
				...
				Root.p[0].GET_SKIN().label.fontSize = oldL;
				var t = titleWin;
				titleWin = null;
