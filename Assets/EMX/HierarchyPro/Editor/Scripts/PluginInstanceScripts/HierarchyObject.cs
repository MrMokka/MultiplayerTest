...
#pragma warning restore
		...
		internal Vector2 GetContentSize()
		{
			...
			var key = ToString().GetHashCode() ^ Root.p[pluginID].ha.INTERNAL_LABEL_STYLES[0].style.fontSize;
			if (!content_size.TryGetValue(key, out tv2))
			{
				tcont.text = ToString();
				content_size.Add(key, tv2 = Root.p[pluginID].ha.INTERNAL_LABEL_STYLES[0].style.CalcSize(tcont));
			}
			return tv2;
		...
		internal HierarchyObject parent()
		{
			...
			if (!Cache.m_PathToObject.ContainsKey(project.assetFolder))
			{
				...
				var load = Cache.GetHierarchyObjectByGUID(ref guid, ref project.assetFolder, null);
				if (!Cache.m_PathToObject.ContainsKey(project.assetFolder))
		...
		internal bool IsLastSibling()
		{
			...
			if (p._visibleTreeItem != null && p._visibleTreeItem.hasChildren && p._visibleTreeItem.children[0] != null)
			{
		...
		internal int ChildCount()
		{
			...
			if (_visibleTreeItem != null && _visibleTreeItem.children[0] != null) return _visibleTreeItem.children.Count;
			...
			if (!_child_count.ContainsKey(project.assetPath))
			{
				...
				else
				{
					...
					_child_count.Add(project.assetPath, td.Count(d => File.Exists(d + ".meta")) + tf.Count(f => !f.EndsWith(".meta") && f[0] != '~' && f[0] != '.'));
				}
			}
