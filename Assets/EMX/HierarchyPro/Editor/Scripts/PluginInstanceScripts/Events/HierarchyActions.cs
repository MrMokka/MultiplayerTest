...
namespace EMX.HierarchyPlugin.Editor.Events
{
	partial class HierarchyActions
	{
		...
		internal void CLOSE_PREFAB_MODE()
		{
			...
			if (!showingPrefabHeader.GetValue(w.Instance, null).Equals(true)) return;
		...
		internal void TryToInitializeDefaultStyles()
		{
			...
			//#EMX_TODO PingSty;e for 2019.2 or below
			...
			{//PING
				...
				var style = m_Ping.GetValue(Root.p[0].gui_currentTree, null) as GUIStyle;
				//var m_PingStyle = treeViewBaseType.GetField("m_Ping", ~(BindingFlags.Static | BindingFlags.InvokeMethod)) ?? throw new Exception("m_Ping");
				//var m_Ping = m_PingStyle.FieldType.GetField("m_PingStyle", ~(BindingFlags.Static | BindingFlags.InvokeMethod)) ?? throw new Exception("m_PingStyle");
				//var style = m_Ping.GetValue(Root.p[0].gui_currentTree) as GUIStyle;

				FixStyle(style);

							m_Ping.SetValue(Root.p[0].gui_currentTree, style, null);
