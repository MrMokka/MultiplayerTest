...
namespace EMX.HierarchyPlugin.Editor
{
	...
	partial class ProjectWindowObjectsData : ScriptableObject
	{
		...
		const string TypeName = "ProjectWindowObjectsData.asset";
		internal static Func<ProjectWindowObjectsData> Instance = () =>
		{
			...
			return (Instance = () =>
			{
				...
				var loaded = AssetDatabase.LoadAssetAtPath<ProjectWindowObjectsData>(ASSET_PATH);
				if (!loaded)
				{
		...
		internal bool RemoveObjectData(HierarchyObject o)
		{
			...
			path_to_cache.Remove(o.project.guid);
			SetDirty();
			return true;
		}


		internal bool SetObjectData(HierarchyObject o, TempSceneObjectPTR data)
		{
			if (o.project == null) throw new Exception("o.project == null");
			var g = GetObjectData(o);
			if (g == null)
